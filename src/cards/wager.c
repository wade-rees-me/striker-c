...
// Double the bet
void wagerDoubleBet(Wager *wager) { wager->amount_bet *= 2; }

// Check if the wager's hand is a blackjack
bool wagerIsBlackjack(Wager *wager) { return handIsBlackjack(&wager->hand); }
...
void wagerWon(Wager *wager) { wager->amount_won = wager->amount_bet; }

// Mark the wager as lost
void wagerLost(Wager *wager) { wager->amount_won = -wager->amount_bet; }
...
void wagerWonInsurance(Wager *wager) { wager->insurance_won = wager->insurance_bet * 2; }

// Mark insurance as lost
void wagerLostInsurance(Wager *wager) { wager->insurance_won = -wager->insurance_bet; }
