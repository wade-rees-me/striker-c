...
bool handIsBlackjack(Hand *hand) { return hand->card_count == 2 && hand->hand_total == 21; }

// Check if the hand is a pair
bool handIsPair(Hand *hand) { return hand->card_count == 2 && hand->cards[0]->value == hand->cards[1]->value; }

// Check if the hand is a pair of aces
bool handIsPairOfAces(Hand *hand) { return handIsPair(hand) && hand->cards[0]->value == 11; }

// Check if the hand is busted (over 21)
bool handIsBusted(Hand *hand) { return hand->hand_total > 21; }

// Check if the hand has a soft ace
bool handIsSoft(Hand *hand) { return hand->soft_ace > 0; }

// Get the total value of the hand
int handTotal(Hand *hand) { return hand->hand_total; }

// Check if the hand is a soft 17
bool handIsSoft17(Hand *hand) { return hand->hand_total == 17 && handIsSoft(hand); }
