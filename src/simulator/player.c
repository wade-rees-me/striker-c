...
void playerShuffle(Player *player) { memset(player->seen_cards, 0, sizeof(player->seen_cards)); }
...
void playerPlay(Player *player, Shoe *shoe, Card *up, bool mimic) {
    ...
    // Check for double
    if (strategyGetDouble(player->strategy, player->seen_cards, player->wager.hand.hand_total,
                          handIsSoft(&player->wager.hand), up)) {
    ...
    if (handIsPair(&player->wager.hand) &&
        strategyGetSplit(player->strategy, player->seen_cards, player->wager.hand.cards[0], up)) {
    ...
    bool doStand = strategyGetStand(player->strategy, player->seen_cards, player->wager.hand.hand_total,
                                    handIsSoft(&player->wager.hand), up);
    while (!handIsBusted(&player->wager.hand) && !doStand) {
        playerDrawCard(player, &player->wager.hand, shoe);
        if (!handIsBusted(&player->wager.hand)) {
            doStand = strategyGetStand(player->strategy, player->seen_cards, player->wager.hand.hand_total,
                                       handIsSoft(&player->wager.hand), up);
        }
    }
}
...
void playerPlaySplit(Player *player, Wager *wager, Shoe *shoe, Card *up) {
    if (handIsPair(&wager->hand) && player->split_count < MAX_SPLIT_HANDS &&
        strategyGetSplit(player->strategy, player->seen_cards, wager->hand.cards[0], up)) {
        Wager *split = &player->splits[player->split_count];
        player->split_count++;
        player->report.total_splits++;
    ...
    bool doStand =
        strategyGetStand(player->strategy, player->seen_cards, wager->hand.hand_total, handIsSoft(&wager->hand), up);
    while (!handIsBusted(&wager->hand) && !doStand) {
        playerDrawCard(player, &wager->hand, shoe);
        if (!handIsBusted(&wager->hand)) {
            doStand = strategyGetStand(player->strategy, player->seen_cards, wager->hand.hand_total,
                                       handIsSoft(&wager->hand), up);
        }
    }
}
...
//
void playerShowCard(Player *player, Card *card) { player->seen_cards[card->value]++; }
