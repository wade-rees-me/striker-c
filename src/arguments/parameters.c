...
Parameters *newParameters(Arguments *arguments) {
	...
	parameters->share_of_hands = parameters->number_of_hands / parameters->number_of_threads;
...
void printParameters(const Parameters *parameters) {
	...
	printf("    %-26s: %17s\n", "Thread's share of hands", convertToStringWithCommas(parameters->share_of_hands, buffer, MAX_BUFFER_SIZE));
